% =====================================================================
% PARTE IV — Indice (ceri e decisioni di design)
% =====================================================================
\part{Indice}
...
\midrule
$\tto, \ff, \zero, \suc\,n, \refl, \nil, \cons$ & linguaggio & s\`i, sono termini (introdotti dalle regole I) \\
$\mathbb{B}, \mathbb{N}, \Pi, \Sigma, \_\identita\_, \mathrm{Vec}\,A\,n, \bot$ & linguaggio & s\`i, sono tipi (introdotti dalle regole F) \\
$\indB, \indN, \indS, \Jelim$, applicazione di $\Pi$ & metalinguaggio & no, sono regole (E) \\
Regole di computazione (Cero~\ref{cero:comp}) & metalinguaggio & no, sono definitorie (C) \\
$\equiv$ definitorio (giudizio) & metalinguaggio & no, \`e un'affermazione \\
...
\section*{Per i prossimi capitoli di Stump}

Questo libro copre i prerequisiti per il capitolo~1 di \emph{Verified Functional Programming in Agda} e srotola un esercizio rappresentativo (\S\ref{cap:and-comm}). Lo schema F-I-E-C generalizza facilmente:

\begin{itemize}
\item \textbf{Capitolo 2 di Stump --- numeri naturali.} L'eliminazione di $\mathbb{N}$ (\S\ref{cap:nat}) entra in gioco con la sua \emph{ricorsione}: le prove non si chiudono pi\`u solo con $\refl$, ma richiedono di chiamare l'ipotesi induttiva. Aggiungere un capitolo che srotoli, per esempio, l'associativit\`a di \texttt{+} su $\mathbb{N}$: stesso schema di \S\ref{cap:and-comm} con $\indN$ al posto di $\indB$ e l'ipotesi induttiva esplicitamente visibile come chiamata ricorsiva.

\item \textbf{Capitoli successivi --- liste, parsing, programmi verificati.} Ogni tipo che si incontra avr\`a le sue quattro regole F-I-E-C. Ogni esercizio sar\`a leggibile come intreccio di Eliminazioni (case-split / induzione), Computazioni (riduzioni definitorie) e Introduzioni (costruzione del termine finale).
\end{itemize}

\noindent Il vocabolario \`e fissato. Da qui in poi si aggiungono tipi, non strumenti.
