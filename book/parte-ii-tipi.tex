...
\noindent Le due viste sono equivalenti: il nostro schema $\mathsf{fz}\,n : \mathrm{Fin}\,(\suc\,n)$ \`e ci\`o che si deriva dalla Π-eliminazione di $\mathsf{fz} : (n : \mathbb{N}) \to \mathrm{Fin}\,(\suc\,n)$ applicato a un $n$ generico. Differiscono solo in \emph{dove} tieni il ``per ogni'': meta nello schema, oggetto nel Π di Agda.

\medskip

\noindent La sintassi \texttt{data ... where} riflette esattamente la scelta Π-internalizzata:
...
\begin{itemize}
\item \texttt{fz : ∀ \{n\} → Fin (suc n)} \emph{\`e} il Π $(n : \mathbb{N}) \to \mathrm{Fin}\,(\suc\,n)$ della costante $\mathsf{fz}$, con $n$ implicito. Il \texttt{∀} \`e zucchero per $\Pi$ (\S\ref{cap:forall}); le graffe \texttt{\{n\}} segnalano che $n$ vive nella passata 1 (\S\ref{cap:pi-azione}) ed \`e inferito dal tipo target.
...
\noindent\textbf{Da fissare.} Lo schema di inferenza tiene il ``per ogni $n$'' nel metalinguaggio --- coerente con il resto del libro (vedi $\suc$, $\cons$). Agda lo \emph{internalizza} come Π di prima classe sul costruttore. Stessa matematica, due punti di vista; la sintassi \texttt{∀ \{n\}} \`e il segnale che Agda ha scelto la seconda strada.
