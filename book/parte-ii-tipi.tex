...
\section*{I --- Introduzione}
\[
\inferrule{n : \mathbb{N}}{\mathsf{fz} : \mathrm{Fin}\,(\suc\,n)}
\qquad
\inferrule{i : \mathrm{Fin}\,n}{\mathsf{fs}\,i : \mathrm{Fin}\,(\suc\,n)}
\]

Entrambi i costruttori concludono in $\mathrm{Fin}\,(\suc\,n)$: nessuno produce un $\mathrm{Fin}\,0$. Conseguenza: $\mathrm{Fin}\,0$ \`e abitato da \emph{zero} termini --- senza essere $\bot$, e senza una regola dedicata. Lo si scopre tentando di costruirne uno: nessuno dei due costruttori \`e applicabile, perch\'e nessuno ha $\zero$ come indice di conclusione.

\medskip

\noindent Viceversa, $\mathrm{Fin}\,3$ ha esattamente tre abitanti: $\mathsf{fz}$, $\mathsf{fs}\,\mathsf{fz}$, $\mathsf{fs}\,(\mathsf{fs}\,\mathsf{fz})$. La taglia del tipo si legge nell'indice.
...
Le graffe in \texttt{∀ \{n\}} sono il riconoscimento sintattico di \S\ref{cap:pi-azione}: $n$ vive nella passata 1 ed \`e inferito dal tipo del risultato di ciascun costruttore.
