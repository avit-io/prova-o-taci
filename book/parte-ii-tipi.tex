...
\noindent Una premessa di forma, prima delle regole. \textbf{I costruttori di una famiglia indicizzata sono formalmente di tipo $\Pi$}: prendono l'indice come argomento e producono un abitante della famiglia in quel punto. Per $\mathrm{Fin}$ ci sono due costruttori, e ciascuno \`e una \emph{costante del linguaggio con un tipo $\Pi$}:
\[
\inferrule{ }{\mathsf{fz} : (n : \mathbb{N}) \to \mathrm{Fin}\,(\suc\,n)}
\qquad
\inferrule{ }{\mathsf{fs} : (n : \mathbb{N}) \to \mathrm{Fin}\,n \to \mathrm{Fin}\,(\suc\,n)}
\]

\noindent Niente abuso: $\mathsf{fz}$ non \`e una costante che ``vive in tutti i $\mathrm{Fin}\,(\suc\,n)$'' --- \`e una funzione di tipo $\Pi$ che, applicata a un $n$, produce un abitante di $\mathrm{Fin}\,(\suc\,n)$. Quando in pratica si scrive $\mathsf{fz} : \mathrm{Fin}\,3$ si sta usando $\mathsf{fz}\,2$, e il typechecker accetta perch\'e $\mathrm{Fin}\,(\suc\,2) \definitorio \mathrm{Fin}\,3$.

\medskip

\noindent Entrambi i costruttori concludono in $\mathrm{Fin}\,(\suc\,n)$: nessuno ha $\mathrm{Fin}\,\zero$ come tipo di ritorno. Conseguenza: $\mathrm{Fin}\,0$ \`e abitato da \emph{zero} termini, senza essere $\bot$ e senza una regola dedicata. Per costruirne uno servirebbe applicare $\mathsf{fz}\,n$ o $\mathsf{fs}\,n\,i$ con il tipo di ritorno definitoriamente uguale a $\mathrm{Fin}\,0$, cio\`e $\suc\,n \definitorio 0$: impossibile per ogni $n$.
...
\noindent\textbf{Lettura della sintassi \texttt{data\,...\,where}.} Agda elenca i costruttori \emph{con il loro tipo $\Pi$ scritto per esteso} --- la sintassi non introduce una regola nuova rispetto a quanto visto in \S\ref{cap:pi}--\S\ref{cap:pi-azione}. Tradotti pezzo per pezzo:
\begin{itemize}
\item \texttt{fz : ∀ \{n\} → Fin (suc n)} \`e esattamente $(n : \mathbb{N}) \to \mathrm{Fin}\,(\suc\,n)$. Il \texttt{∀} \`e zucchero per $\Pi$ (\S\ref{cap:forall}); le graffe \texttt{\{n\}} segnalano che $n$ vive nella passata 1 (\S\ref{cap:pi-azione}) e viene inferito dal tipo target. Senza graffe avremmo \texttt{fz : (n : ℕ) → Fin (suc n)} e in codice andrebbe scritto \texttt{fz 2} a mano.
\item Quindi nel codice ``\texttt{fz : Fin 3}'' \`e l'elaborazione di \texttt{fz \{2\}}: Agda guarda il target \texttt{Fin 3}, unifica $\suc\,n \definitorio 3$, deduce $n = 2$ e riempie l'argomento implicito.
\end{itemize}

\noindent In altre parole: la sintassi \texttt{data ... where} non aggiunge primitive al linguaggio, dichiara solo che \texttt{fz} e \texttt{fs} sono \emph{termini} di certi tipi $\Pi$, e che la formazione di $\mathrm{Fin}\,n$ \`e quella della regola F sopra.
