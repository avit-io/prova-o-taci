...
\noindent\textbf{Cosa non sono i ceri.} Non sono assiomi nel senso di Hilbert (proposizioni del linguaggio scelte come punto di partenza per derivare altre proposizioni del linguaggio): appartengono al metalinguaggio, o sono scelte di design. Non sono congetture (non si tratta di affermazioni che potrebbero un giorno rivelarsi false). Non sono postulati arbitrari: sono i punti in cui la materia del sistema chiede di essere fissata, e ogni fissaggio \`e una scelta motivata ma non costretta.
...
\section*{Convenzione $\Gamma$ implicito --- come si applica}

La convenzione \`e gi\`a stata dichiarata nel patto di lettura: $\Gamma$ implicito ovunque, esplicito solo quando una premessa lo \emph{estende}. Due esempi operativi.

\medskip

\noindent \emph{Senza $\vdash$, contesto invariato.} La regola di introduzione di $\mathbb{B}$ (\S\ref{cap:bool})
\[
\inferrule{ }{\tto : \mathbb{B}}
\]
sta per $\Gamma \vdash \tto : \mathbb{B}$ per ogni $\Gamma$.

\medskip

\noindent \emph{Con $\vdash$, contesto esteso.} La $\Pi$-formazione (\S\ref{cap:pi}):
\[
\inferrule{A : \Type \\ x : A \vdash B\,x : \Type}{(x : A) \to B\,x : \Type}
\]
Per esteso:
...
\]
\noindent La premessa $\Gamma, x : A \vdash B\,x : \Type$ si legge: \emph{estendi $\Gamma$ con l'ipotesi $x : A$; in questo contesto esteso, $B\,x$ \`e un tipo.} Tutto qui.
...
\item \textbf{Notazione dell'universo dei tipi.} Nel corpo del libro scriviamo $\Type$ (e $A : \Type$) per indicare l'universo dei tipi. Nei sorgenti Agda lo stesso oggetto si chiama \agdainl{Set} (e si scrive \texttt{A : Set}). \`E lo stesso universo, solo con due nomi storici diversi: $\Type$ \`e la terminologia MLTT, \texttt{Set} \`e la scelta di Agda. Nessuna differenza concettuale; usiamo le due notazioni rispettivamente nei due contesti.
\end{itemize}
